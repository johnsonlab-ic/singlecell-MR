#### the following script assumes you have downloaded the pQTL data from Robins et al. (2021) ("ROSMAP_DLPFC_pQTLs.csv")
#https://www.synapse.org/#!Synapse:syn24172458
#https://doi.org/10.1016/j.ajhg.2021.01.012
...
brain_pqtl_path="/path_to_brain_pqtl_analysis/ROSMAP_DLPFC_pQTLs.csv"
...
#######################################
#### 2. READ IN pQTL summary stats ####
#######################################



source("/rds/general/user/ah3918/home/johnson_lab/essential_functions/gene_expression_related/essential_functions.r")

wingo_pqtl=read.csv(pqtl)

wingo_pqtl=wingo_pqtl %>% mutate(chrpos=paste0(CHR,":",POS))
wingo_pqtl$SNP=chrpos_to_rsid(wingo_pqtl$chrpos,build="hg37")

#the uniprot to gene conversion was done online; https://www.uniprot.org/id-mapping 

uniprot_entrez=read.table("/analysisdir/PROTEIN_QTL/PQTL/uniprot_to_entrezid.txt",head=T)
entrez_to_gene=read.csv("/analysisdir/PROTEIN_QTL/PQTL/idmap.csv")

colnames(uniprot_entrez)=c("Uniprot_ID","entrezgene")

conversion=entrez_to_gene %>%
  left_join(uniprot_entrez, by = "entrezgene") %>%
  dplyr::select(Uniprot_ID,symbol)

wingo_pqtl=wingo_pqtl %>% left_join(conversion,by=c("UNIPROT"="Uniprot_ID"),relationship = "many-to-many")
...
gene=mrdf$gene[i]
  chrom = paste0("chr",mrdf$chrom[i])
  trait=mrdf$GWAS[i]
  celltype=mrdf$celltype[i]
  message(paste("Processing gene ",gene,"and trait ",trait))

  tmp_pqtl=wingo_pqtl %>% filter(symbol==gene) 
    
  if(nrow(tmp_pqtl)==0){
          MR_list[[i]]="No protein"
          message("No protein in dataset")
          next
        }
 ...
 region_snps=object@gwas[[regionName]]@beta$rsid
  ...
  tmp_pqtl=tmp_pqtl %>% filter(FDR<0.05,SNP %in% region_snps)

  if(nrow(tmp_pqtl)==0){
          MR_list[[i]]="No significant pQTLs"
          message("No significant p-values found for gene ",gene, " and trait ",trait)
          next
        }
    ...
    merged_df <- gwas %>% mutate(SNP=rsid) %>%
    dplyr::rename(gwas_REF = A1, gwas_EFFECT = A2, gwas_beta = b, gwas_se = se, gwas_pval = pval) %>%
    inner_join(tmp_pqtl %>%
                dplyr::rename(pqtl_REF = REF, pqtl_EFFECT = ALT, pqtl_beta = BETA, pqtl_se = SE, pqtl_pval = P,pqtl_FDR=FDR),
                by = "SNP") %>% dplyr::select(rsid,SNP, pqtl_REF, pqtl_EFFECT, pqtl_beta, 
                pqtl_se, pqtl_pval,pqtl_FDR, 
...
names(MR_list)=paste0(mrdf$gene,"_",mrdf$GWAS)
...
saveRDS(tmpdf,paste0(pqtl_analysis_dir,"/MR_results_Wingo.rds"))
write.xlsx(tmpdf,paste0(tabledir,"Suppl.Table.11_pQTL_Wingo_MR_results.xlsx"))